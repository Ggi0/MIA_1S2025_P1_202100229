...
atributos:
	...
	dsk_fit - char     - char         - Tipo de ajuste de la partición. B (Best), F (First) o W (worst)
	mbr_partitions     - partition[4] - Estructura con información de las 4 particiones
*/
type MBR struct {
	Mbr_tamanio        int32        // Tamaño del DISCO en bytes
	Mbr_creation_date  [16]byte     // Fecha y hora de creación del MBR
...
func EscribirMBR(file *os.File, tam int, fit string) (*os.File, error) {
	...
	copy(newMBR.Mbr_creation_date[:], ahora.Format("02/01/2006 15:04"))
